...
main_log_folder=config.Logs_Folder_Path+'/health_check_hardware_RFID_Reader_status/'
json_path=config.HealthCheck_Hardware_Json

def check_RFID_Reader_status(report):
    # for i in range(1,int(LaneCount)+1):
    report['status']='On'
    return report
...
def main():
    ...
    for i in range(LaneCount):
        ...
        report=check_RFID_Reader_status(report)
        final_report.append(report)
    base_data['rfidportdetails']=final_report
