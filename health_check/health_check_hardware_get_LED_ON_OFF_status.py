...
main_log_folder=config.Logs_Folder_Path+'/hardware_led_on_off_status/'
json_path=config.HealthCheck_Hardware_Json

def check_led_on_off_status(report):
...
def main():
    ...
    for i in range(LaneCount):
        ...
        report=check_led_on_off_status(report)
        final_report.append(report)
    base_data['ledlightstatus']=final_report
